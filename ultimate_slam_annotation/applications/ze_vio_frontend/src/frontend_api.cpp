...
void VisualOdometry::subscribeDataProviders()
{
  ...
  if (!FLAGS_vio_use_events_and_images) {
    //如果满足if，则vio_use_events_and_images = 0，表示不同时使用e+i
    if (!FLAGS_vio_use_events) {
      //若满足if，则vio_use_event = 0，则表示不使用event，此时为vio
      ...
      //不满足if，则vio_use_event = 1，则表示使用event，此时为eio
